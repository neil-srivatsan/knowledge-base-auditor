"""SQLite persistence for scan history and document fingerprints."""
...
from kb_audit.models import AuditResult, Document, Severity, StalenessSignal, WorkflowState
...
#: Singleton sentinel.  Import alongside Database when you need it.
...
class ScanLeaseContext:
    ...
    def __init__(self, db: Database, owner_token: str) -> None:
        self._db = db
...
class Database:
    ...
    def store_result(
        ...
        trust_data_json = json.dumps({
            "metadata": result.trust_metadata,
            "evidence": result.trust_evidence,
        })
    ...
    def load_audit_results(self, scan_id: int, doc_ids: list[str]) -> list[AuditResult]:
        ...
        for r in rows:
            ...
            raw_signals: list[dict] = json.loads(r[5]) if r[5] else []
            signals = [
                StalenessSignal(
                    signal_type=s["signal_type"],
            ...
            trust_data = json.loads(r[8]) if r[8] else {}
    ...
    def get_scan_results(self, scan_id: int) -> list[dict]:
        ...
        results = []
        for r in rows:
            trust_data = json.loads(r[10]) if r[10] else {}
            result = {
                "id": r[0],
                "title": r[1],
                "url": r[2],
